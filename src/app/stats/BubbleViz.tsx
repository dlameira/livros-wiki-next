...
type SimNode = d3.SimulationNodeDatum & BubbleNode & { r: number; color: string; vizGrupo: string }

const INDEP_LABEL = 'independentes'
...
export default function BubbleViz({ data }: Props) {
  ...
  // All vizGrupos: named groups + independentes as one cluster
  const allVizGrupos = (() => {
    const named = [...new Set(data.filter(d => d.grupo).map(d => d.grupo))].sort((a, b) => {
      const sa = data.filter(d => d.grupo === a).reduce((s, d) => s + d.count, 0)
      const sb = data.filter(d => d.grupo === b).reduce((s, d) => s + d.count, 0)
      return sb - sa
    })
    if (data.some(d => !d.grupo)) named.push(INDEP_LABEL)
    return named
  })()
  ...
  const draw = useCallback(() => {
    ...
    const rScale = d3.scaleSqrt().domain([0, maxCount]).range([3, Math.min(width, 800) * 0.038])

    const nodes: SimNode[] = data.map(d => ({
      ...d,
      vizGrupo: d.grupo || INDEP_LABEL,
      r: rScale(d.count),
      color: getColor(d.grupo || INDEP_LABEL),
      x: width / 2 + (Math.random() - 0.5) * width * 0.4,
      y: 300 + (Math.random() - 0.5) * 200,
    }))

    // Estimate cluster radius per vizGrupo
    const groupSizes = allVizGrupos.map(g => {
      const gn = nodes.filter(d => d.vizGrupo === g)
      const totalArea = gn.reduce((s, d) => s + Math.PI * d.r * d.r, 0)
      const clusterR = Math.sqrt(totalArea / Math.PI) * 1.6 + 16
      return { name: g, clusterR, count: gn.length }
    })
    ...
    let cx = margin, cy = margin + 24, rowH = 0
    ...
    const height = Math.max(400, cy + rowH + margin + 20)
    ...
    const circles = circleLayer.selectAll<SVGCircleElement, SimNode>('circle')
      ...
      .on('mouseleave', (event, d) => {
        d3.select(event.currentTarget)
          .attr('opacity', selected && d.vizGrupo !== selected ? 0.08 : 0.8)
          .attr('stroke', '#fff').attr('stroke-width', 1)
        if (tooltip) tooltip.style.opacity = '0'
      })

    const bLabels = labelLayer.selectAll<SVGTextElement, SimNode>('text')
      .data(nodes.filter(d => d.r > 16)).join('text')
    ...
    // Wiggle force for organic motion
    let tick = 0
    function wiggleForce() {
      return () => {
        ...
        const t = tick * 0.006
        for (const n of nodes) {
          n.vx! += Math.sin(t + (n.index || 0) * 0.5) * 0.015
          n.vy! += Math.cos(t * 0.7 + (n.index || 0) * 0.3) * 0.012
        }
      }
    }

    const sim = d3.forceSimulation<SimNode>(nodes)
      .force('x', d3.forceX<SimNode>(d => clusterCenters[d.vizGrupo]?.x || width / 2).strength(0.35))
      .force('y', d3.forceY<SimNode>(d => clusterCenters[d.vizGrupo]?.y || 300).strength(0.35))
    ...
    function update() {
      ...
      allVizGrupos.forEach(g => {
        const gn = nodes.filter(n => n.vizGrupo === g)
  ...
  useEffect(() => {
    ...
    svg.selectAll<SVGCircleElement, SimNode>('.circles circle')
      ...
      .attr('opacity', d => selected && d.vizGrupo !== selected ? 0.08 : 0.8)
    ...
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '0 32px 20px', alignItems: 'center' }}>
        ...
        {allVizGrupos.map(g => {
          ...
          const total = data.filter(d => (d.grupo || INDEP_LABEL) === g).reduce((s, d) => s + d.count, 0)
          return (
